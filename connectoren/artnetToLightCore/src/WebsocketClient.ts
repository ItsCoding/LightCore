...
export class WebSocketClient {
    ...
    private handleMessage(message: string): void {
        ...
        // console.log("Received message: ", topic.type);
    ...
    public async setStripBrightness(stripIndex: number, brightness: number) {
        if (this.socket || this.inTransaction) {
            this.send("light.setStripBrightness", { stripIndex: stripIndex, brightness: brightness });
        }
    }

    public async setColorPaletteRaw(colors: number[][]) {
        if (this.socket || this.inTransaction) {
            this.send("light.colorPalette.set", { colorPalette: colors });
        }
    }

    public async setStripSpeed(stripIndex: number, speed: number) {
        if (this.socket || this.inTransaction) {
            this.send("light.setStripSpeed", { stripIndex: stripIndex, speed: speed });
        }
    }

    public async setStripIntensity(stripIndex: number, intensity: number) {
        if (this.socket || this.inTransaction) {
            this.send("light.setStripIntensity", { stripIndex: stripIndex, intensity: intensity });
        }
    }
